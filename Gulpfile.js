...
const jsFiles = [config.jsSrc + "/main.js", config.jsSrc + "/links.js"];
...
function injectCss() {
    ...
    return gulp
        .src(config.tplDir + config.headerTpl)
...
function injectJs() {
    ...
    return gulp
        .src(config.tplDir + config.footerTpl)
...
function build(done) {
    "use strict";
    if (config.injectAssets) {
        return gulp.series(gulp.parallel(gulp.series(css, injectCss, cleanCss)), gulp.parallel(gulp.series(js, compress, injectJs, cleanJs)))(done);
    } else {
        return gulp.series(gulp.parallel(gulp.series(css, cleanCss)), gulp.parallel(gulp.series(js, compress, cleanJs)))(done);
    }
}
